...
test('P2020-01/02 / PHOTO-01/02: generated page has six highlights and complete archive of 23 photos', async ({page}) => {
  await page.goto('/photos/');
  await expect(page.locator('[data-photo-slide]')).toHaveCount(6);
  await expect(page.locator('.photos-archive figure')).toHaveCount(23);
  await expect(page.locator('.photos-archive img[alt="Image description"]')).toHaveCount(0);
});
test('P2020-06 / PHOTO-16: archive links work with JavaScript disabled', async ({browser}) => {
